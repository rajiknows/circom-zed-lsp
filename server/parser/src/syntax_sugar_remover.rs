...
use program_structure::statement_builders::{
    build_assert, build_constraint_equality, build_declaration, build_initialization_block,
    build_log_call, build_return,
};
...
pub fn apply_syntactic_sugar(program_archive: &mut ProgramArchive) -> Result<(), Report> {
    ...
    for temp in program_archive.templates.clone() {
        ...
        let (new_body, initializations) = remove_anonymous_from_statement(
            &mut program_archive.templates,
        ...
        if let Statement::Block { meta, mut stmts } = new_body {
            let (component_decs, variable_decs, mut substitutions) =
                separate_declarations_in_comp_var_subs(initializations);
            ...
            let t2 = TemplateData::copy(
                t.get_name().to_string(),
                t.get_file_id(),
                new_body,
                t.get_num_of_params(),
                t.get_name_of_params().clone(),
                t.get_param_location(),
                t.get_inputs().clone(),
                t.get_outputs().clone(),
                t.is_parallel(),
                t.is_custom_gate(),
                t.get_declaration_inputs().clone(),
                t.get_declaration_outputs().clone(),
            );
...
fn remove_anonymous_from_statement(
    ...
    match stm {
        ...
        Statement::While { meta, cond, stmt } => {
            ...
            let (while_ok, mut declarations2) = remove_anonymous_from_statement(
                templates,
                file_lib,
                *stmt,
        ...
        Statement::LogCall { meta, args } => Result::Ok((build_log_call(meta, args), Vec::new())),
        Statement::Assert { meta, arg } => Result::Ok((build_assert(meta, arg), Vec::new())),
        Statement::Return { meta, value: arg } => Result::Ok((build_return(meta, arg), Vec::new())),
        Statement::ConstraintEquality { meta, lhe, rhe } => {
    ...
    match exp {
        AnonymousComp {
            ...
            if template.is_none() {
                return Result::Err(anonymous_general_error(
                    meta.clone(),
                    "The template does not exist ".to_string(),
                ));
            }
            ...
            for (name_signal, _) in inputs {
                ...
                let subs = Statement::Substitution {
                    ...
                    access: acc,
                    op: *new_operators.get(num_input).unwrap(),
                    rhe: new_exp,
                };
                num_input += 1;
...
pub fn separate_declarations_in_comp_var_subs(
