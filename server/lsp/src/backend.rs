...
use crate::ast;
use crate::constansts::DUMMY_MAIN_NAME;
use crate::constansts::LATEST_VERSION;
        ...
        let archive = if publish_diagnostics {
            let (_tmp, file_name, version_string) = {
                    ...
                    })
                {
                    ...
                    let version_string =
                        parse::version_string(ast.get_version().unwrap_or_else(|| LATEST_VERSION));
                    ...
                    (None, main_file_name, parse::version_string(LATEST_VERSION))
                }
            };
...
impl LanguageServer for Backend {
    ...
    async fn hover(&self, params: HoverParams) -> jsonrpc::Result<Option<Hover>> {
        ...
        let Ok(Some((pos, word))) = parse::find_word(
        ...
        let Some(OptionalDocumentData {
            archive,
            main_file_id,
        }) = &document_data.optional
        else {
            return Ok(Some(lsp_types_util::simple_hover(String::from(
                "Could not find information (are there any compilation errors?)",
            ))));
        };
...
fn produce_main(
    file_name: &str,
    ast: &program_structure::ast::AST,
    output_dimensions: HashMap<usize, usize>,
) -> String {
    let version = ast.get_version().unwrap_or_else(|| LATEST_VERSION);
    let version = parse::version_string(version);

    let mut result = format!(
        "pragma circom {};include \"{}\";template {}() {{",
        version, file_name, DUMMY_MAIN_NAME
    );
    for (i, definition) in ast.definitions.iter().enumerate() {
        let (name, args, var_type) = match definition {
            program_structure::ast::Definition::Template { name, args, .. } => {
                (name, args, "component")
            }
            program_structure::ast::Definition::Function { name, args, .. } => (name, args, "var"),
        };
        ...
        for ast_node in ast::ASTIteratorBFS::new(ast::StatementOrExpression::Statement(
            ...
            if let ast::StatementOrExpression::Expression(
                program_structure::ast::Expression::Variable { name, access, .. },
            ) = ast_node
            {
    ...
    write!(result, "}} component main = {}();", DUMMY_MAIN_NAME).unwrap();
        ...
        circom_parser::run_parser(tmp_file_name.to_owned(), version_string, vec![])
    {
        ...
        for report in reports {
            ...
            if report_file_name == tmp_file_name {
                if let program_structure::error_code::ReportCode::WrongTypesInAssignOperationDims(
                    _,
                    expected,
                ) = report_code
                {
